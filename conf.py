...
# ------------------------------- DO-ALL options -------------------------- #
# DO-ALL (Distill Once, Adapt Life-Long): plug-in for any base method, on with --do_all.
_C.DOALL = CfgNode()

_C.DOALL.ENABLED = False            # set by --do_all
_C.DOALL.SYNDATA_PATH = ""          # DD source-anchor ImageFolder, set by --synpath (.../IPC_<N>/<exp>)
_C.DOALL.STRIDE = 1                 # run the anchor branch every STRIDE steps, set by --stride
# anchor objective weights:  L = LAMBDA_ANCHOR * (W_CE*KD + W_MMD*MMD + W_MIX*MixUp)
_C.DOALL.LAMBDA_ANCHOR = 1.0
_C.DOALL.W_CE = 1.0
_C.DOALL.W_MMD = 10.0
_C.DOALL.W_MIX = 1.0
# harm-adaptive blending:
_C.DOALL.SCORE_TYPE = "second_order"  # "first_order" | "second_order"
_C.DOALL.GROUP_BY = "layer"           # "layer" | "param"
_C.DOALL.PERCENTILE = 0.5             # rewind the top-(1-PERCENTILE) harmful groups
_C.DOALL.BETA_MAX = 0.05              # max blend strength toward source
_C.DOALL.BETA_SCALE = 5.0             # sigmoid sharpness of the blend
...
def load_cfg_from_args(description="Config options."):
    ...
    parser.add_argument("--do_all", action="store_true",
                        help="Enable the DO-ALL plug-in on top of the base method")
    parser.add_argument("--synpath", type=str, default="",
                        help="DD source-anchor ImageFolder path (.../IPC_<N>/<exp>); required with --do_all")
    parser.add_argument("--stride", type=int, default=1,
                        help="Run the DO-ALL anchor branch every STRIDE steps")
    ...
    # --- DO-ALL plug-in wiring (CLI overrides config) ---
    if args.do_all:
        cfg.DOALL.ENABLED = True
    if args.synpath:
        cfg.DOALL.SYNDATA_PATH = args.synpath
    cfg.DOALL.STRIDE = args.stride
    if cfg.DOALL.ENABLED and not cfg.DOALL.SYNDATA_PATH:
        raise ValueError("--do_all requires --synpath <path>/IPC_<N>/<exp>")
