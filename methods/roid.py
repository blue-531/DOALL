...
from methods.do_all import DOALL
...
class ROID(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        # DO-ALL plug-in (enabled with --do_all); None = plain ROID baseline.
    ...
    def forward_and_adapt(self, x):
        # DO-ALL: anchor-guided update + harm-aware rewind before the base ROID step.
        if self.do_all is not None and self.do_all.should_run():
            self.do_all.anchor_update(x)
