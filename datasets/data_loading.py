...
def get_test_loader(setting: str, adaptation: str, dataset_name: str, preprocess: Union[transforms.Compose, None],
    ...
    if domain_name == "none":
        ...
        if dataset_name in ["cifar10_c", "cifar100_c"]:
            ...
            # CCC streams as a web-dataset. Use a local copy if present at
            # <DATA_DIR>/<domain_name>/serial_*.tar (point cfg.DATA_DIR at its parent),
            # otherwise stream the official shards over HTTPS.
            # domain_name e.g.: baseline_<0/20/40>_transition+speed_<1000/2000/5000>_seed_<43/44/45>
            local_ccc = os.path.join(data_dir, domain_name)
            local_shards = sorted(glob.glob(os.path.join(local_ccc, "serial_*.tar")))
            if local_shards:
                url = local_shards
                logger.info(f"Using local CCC copy: {len(local_shards)} shards at {local_ccc}")
            else:
